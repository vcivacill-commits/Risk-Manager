...
class RiskEngine:
    ...
    def calculate_setup(self, symbol: str, direction: str, entry_price: float,
                        risk_percent: float = DEFAULT_RISK_PERCENT, timeframe: str = "1h") -> TradeSetup:
        ...
        return TradeSetup(
            ...
            entry_price=round(entry_price, 8), stop_loss=round(stop_loss, 8),
            take_profit=round(take_profit, 8), atr=round(atr, 8),
            risk_percent=risk_percent, leverage=leverage,
...
def register_handlers():
    ...
    async def process_symbol(message: Message, state: FSMContext):
        ...
        await message.answer(
            ...
            parse_mode="HTML", reply_markup=get_direction_keyboard()
        )
    ...
    async def process_entry(message: Message, state: FSMContext):
        ...
        await message.answer(
            ...
            parse_mode="HTML", reply_markup=get_risk_keyboard()
        )
    ...
    async def process_risk(callback: CallbackQuery, state: FSMContext):
        ...
        await callback.message.edit_text(
            ...
            parse_mode="HTML", reply_markup=get_timeframe_keyboard()
        )
        await callback.answer()
    ...
    async def process_timeframe(callback: CallbackQuery, state: FSMContext):
        ...
        try:
            setup = risk_engine.calculate_setup(
                ...
                entry_price=data["entry_price"], risk_percent=data["risk_percent"],
                timeframe=timeframe,
            )
